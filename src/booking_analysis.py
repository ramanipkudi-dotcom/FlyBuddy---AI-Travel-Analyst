"""
FlyBuddy - Booking Lead-Time Analysis Module (Part 3)
-----------------------------------------------------
...
- Connects EDA directly to traveler decision making.
- Empirically calculates windows from data instead of hard-coding arbitrary rules.
- Clearly states methodology limitations (historical correlation != future guarantee).
"""
...
def calculate_booking_lead_time_stats(df, source=None, destination=None, travel_class=None):
    ...
    labels = ['Last Minute (0-7d)', 'Short Notice (8-14d)', 'Moderate (15-21d)', 'Optimal Window (22-35d)', 'Early Bird (36-50d)', 'Far Advance (50d+)']
    ...
    if not valid_windows.empty:
        ...
        cheapest_window_name = 'Standard Advance (21-35 days)'
        cheapest_median_price = float(subset['Price_clean'].median())

    overall_median = float(subset['Price_clean'].median())
    potential_savings = max(0, round(((overall_median - cheapest_median_price) / overall_median) * 100, 1)) if overall_median > 0 else 0.0
